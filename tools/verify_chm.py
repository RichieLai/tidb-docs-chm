...
def main() -> int:
    ...
    if binary and hhc:
        print("  [提示] 同时存在两种目录源：部分第三方阅读器会把二进制条目并进侧栏")

    if hhc:
        text = reader.read(hhc[0]).decode("gbk", "replace")
    ...
    bom = [n for n in html_files if reader.read(n).startswith(b"\xef\xbb\xbf")]
    print(f"正文编码  : {len(bom)}/{len(html_files)} 个 HTML 带 UTF-8 BOM"
          + ("（阅读器按默认编码打开即可正确显示）" if html_files and len(bom) == len(html_files)
             else ""))
